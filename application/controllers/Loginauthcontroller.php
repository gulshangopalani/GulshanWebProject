		...
		function chkLoginUser(){
           ...
           // print_r($user_data);
			...
			if ($user_data['iType']==0) {
			    ...
			    if($user_data['usetype']==0){					
					$getToken=$this->getACookie();
					$this->Modelchkuser->updateToken($getToken);
					$gettokenArray=$this->Modelchkuser->getToken();					
					$TokenData=$gettokenArray[0]['sessionToken'];
					/*$gettokenArray=$this->Modelchkuser->getToken();
					$TokenData=$gettokenArray[0]['sessionToken'];*/
				}else{
					$gettokenArray=$this->Modelchkuser->getToken();
				...
				$this->session->set_userdata('TokenId', $TokenData);
