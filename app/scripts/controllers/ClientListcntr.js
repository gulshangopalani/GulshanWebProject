...
angular.module('ApsilonApp').controller('ClientListcntr',['$scope','$mdDialog', '$http', 'sessionService', '$timeout', 'deviceDetector','$filter','$rootScope','$window', function ($scope, $mdDialog,$http, sessionService, $timeout, deviceDetector,$filter,$rootScope,$window) {
    ...
    $scope.FreeChipsSubmit = function (Chip, Type, UserID, Free, userType,node) {
                ...
                if (Type==1 && ($scope.pWallet >= Chip.ChipVal)) {
                    $http({
                        method: 'POST',
    ...
    $scope.displayUserInfo=function(UserArray,DealerFreeChip){
        $mdDialog.show({
            controller: displayUserDeatailsCntr,
            templateUrl: 'app/scripts/directives/popupform/userDetails.html?var='+Math.random(),
            locals: { prntScope: $scope, UserData: UserArray,DealerFreeChip:DealerFreeChip },
            clickOutsideToClose: true,
            fullscreen: false,
        });
    }
    function displayUserDeatailsCntr($scope, $mdDialog, prntScope, UserData, sessionService,DealerFreeChip) {
            $scope.displayUndu=false;
            ...
            $scope.updateUserChips=function(UserData){
                $http.get('Chipscntrl/getChipDataById/' + UserData.usecode).success(function (data, status, headers, config) {
                    $scope.cipsData1 = data.betLibility[0];
                });
                
            }
            $scope.updateUserChips(UserData);

            /*start code for chip INOUT*/
                $scope.freechips1 = function (id) {
                    debugger;
                        $http.get('Chipscntrl/getChipDataById/' + sessionService.get('user_id')).success(function (data, status, headers, config) {
                            $scope.cipsData = data.betLibility;
                ...
                /*end of the code Free chips*/
                $scope.node = UserData;
                prntScope.freechips(UserData.usecode);
                $scope.freechips1(UserData.usecode);
                $scope.FreeChipsSubmit = function (Chip, Type, UserID, Free, userType) {
                    debugger;
                    if(Type==1){
                        var Message="Deposit";
                    ...
                    var warning = 'Are you sure you want to '+Message+' this User...';
                    if (confirm(warning)) {
                        prntScope.FreeChipsSubmit(Chip, Type, UserID, Free, userType,$scope.node);
                        Chip.ChipVal = "";
                        Chip.Ref = "";
                        $scope.freechips1(UserID);
                        $scope.updateUserChips($scope.node);
                    }
                }
            /*End the code of chip in out*/
            /*start code for settlement A/c*/ 
            var node=  UserData;             
                $scope.userType = 2;
                ...
                $scope.MainChipsSubmit = function () {
                    debugger;
                    var warning = 'Are you sure you want to settlement of this User...';
                    if (confirm(warning)) {
                       if($scope.node.P_L < 0){
                            $http.get('Chipscntrl/getChipDataById/' + sessionService.get('user_id')).success(function (data, status, headers, config) {
                                ...
                                if(pWallet>=$scope.txtChip){
                                    ...
                                    $http({
                                        ...
                                        debugger;
                                        $scope.displayUndu=true;
                                       /* $mdDialog.hide();*/
                                        alert(data.message);
                                        $scope.TransactionId=data.txnId;
                                        $scope.updateUserChips($scope.node);
                ...
                $scope.rollBack=function(){
                    alert($scope.TransactionId);
                     $http.get('Createmastercontroller/RollBackSettlement/' + $scope.TransactionId).success(function (data, status, headers, config) {
                        alert(data.message);
                     });
                }
            /*End of the code of settlement A/C*/
