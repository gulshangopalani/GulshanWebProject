'use strict';
angular.module('ApsilonApp').controller('ClientListcntr',['$scope','$mdDialog', '$http', 'sessionService','get_userser', '$timeout', 'deviceDetector','$filter', function ($scope, $mdDialog,$http, sessionService,get_userser, $timeout, deviceDetector,$filter) {
    ...
    $scope.FreeChipsSubmit = function (Chip, Type, UserID, Free, userType,node) {
                ...
                if (($scope.pWallet >= Chip.ChipVal)&&(Type==1)) {
                    $http({
                        method: 'POST',
